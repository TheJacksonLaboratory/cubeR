...
get_lod_scan <- function(dataset, id, intcovar = NULL, cores = 0) {
    ...
    logger$info(paste0('scan - time: ', elapsed['elapsed']))
    ...
    #lod <- lod[1:100,] %>% mutate(lod = sprintf("%0.3f", lod))
    inner_join(as_tibble(temp, rownames = 'marker.id'),
               markers,
               by = 'marker.id') %>%
        select(id = marker.id, chr, pos, lod = id) %>% mutate(lod = as.double(lod))
}
...
get_founder_coefficients <- function(dataset, id, intcovar, chrom,
    ...
    if (gtools::invalid(intcovar)) {
        ...
        temp <- as_tibble(temp, rownames = 'marker.id') %>% mutate(
            A = as.double(A), B = as.double(B), C = as.double(C),
            D = as.double(D), E = as.double(E), F = as.double(F),
            G = as.double(G), H = as.double(H), intercept = as.double(intercept))
