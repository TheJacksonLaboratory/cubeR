...
var _API_R_BASE = 'http://localhost/api-102/';
...
`;
    ...
    function downloadChromosomes(release, species, callback) {
        ...
        let chromosomeURL = window.location.protocol + '//' + window.location.host + '/ensimpl/api/chromosomes'
        let amp = '';
        ...
        chromosomeURL = 'static/data/chromosomes.json';
...
//        let genesURL = `${g.PROTOCOL}//churchilllab.jax.org/ensimpl/api/search?species=${g.SPECIES_ID}&term=${chromosome}:${minPos}-${maxPos}`;
        ...
        let genesURL = window.location.protocol + '//' + window.location.host + '/ensimpl/api/search?species='
        ...
        $.when( 
                ...
                $.ajax({type: 'GET',
           		    	url: genesURL})
            ).done(function( responseSnpAssoc, responseGenes ) {
            	plotSNPAssoc(responseSnpAssoc[0].result,
            			responseGenes[0].result,
...
//        let mediateURL = `http://10.105.9.102/api/get/http://localhost:8001/mediate?dataset=${g.dataSetID}&id=${id}&marker_id=${markerID}&dataset_mediate=${mediateAgainst}`;
		...
		//          updateMediationData(data.group_id, mediateAgainst);
		          plotMediation(data.result, mediateAgainst);
		          stopTask();
		      },
		      error: function(jqXHR, textStatus, errorThrown) {
		          showErrorMessage(errorThrown, textStatus);
...
//        let lodURL = `http://10.105.9.102/api/get/http://localhost:8001/lodscansamples?dataset=${g.dataSetID}&id=${id}&chrom=${chromosome}&intcovar=${covar}`;
        ...
        $.when( 
                ...
                $.ajax({type: 'GET',
              		    url: lodURL})
            ).done(function( responseEffect, responseLod ) {
            	updateEffectData2(responseEffect[0], responseLod[0], chromosome, covar);
            });
...
// cccc
    function updateEffectData2(effect, lodSamples, chromosome, covar) {
        // show result, there will be 1 datasets to get
        if (covar === 'additive') {
            plotEffectData(effect.result, null, chromosome, covar);
        } else {
            plotEffectData(effect.result,
                           lodSamples.result,
                           chromosome, covar);
        }
...
// cccc
        ...
        $.when( 
            ...
            $.ajax({type: 'GET',
      		    	url: urlLOD})
        ).done(function( responseCorrelation, responseExpression, responseLod ) {
        	updatePhenotypeSelect2(covar, responseCorrelation[0], responseExpression[0], responseLod[0]);
        });
