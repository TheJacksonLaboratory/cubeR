...
function init() {
    ...
    Shiny.addCustomMessageHandler('js_action_handler', (action) => {
        log("Shiny Returned: " + action.id + ' ' + action.description );
    //    log(action)
        var data = JSON.parse(action.data[0])
        var callback = _handler_callback[action.id + ''];
        if ( callback != null ) {
            callback(null, data)
        }
    });
